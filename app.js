...
function deepClone(obj) {
    if (obj === undefined) return undefined; if (obj === null) return null;
...
function calculate_state(get_events_function, initial_state, event_handlers) {
    ...
    get_events_function((event) => {
        if (event_handlers[event.name]) { state = event_handlers[event.name](state, event); }
    }, (error) => { console.error("Error getting events: " + error.message); });
    return state;
}
...
function execute_slice_logic({
    slice, parameter_function, on_error, on_event_success, on_exception, on_query, on_unknown
}) {
    let state_function = undefined;
    try {
        console.log("2.0 setting up calculating state function");
        state_function = () => calculate_state(get_events, slice.initial_state, slice.event_handlers);
    } catch (error) {
        console.error("2.1 Error setting up calculating state function: " + error.message);
        if (on_error) return on_error(error, 500); else return;
    }

    let result = undefined;
    try {
        console.log("3.0 calling refinement function");
        result = slice.refinement_function(state_function, parameter_function);
    } catch (error) {
        console.error("3.1 Error invariant function: " + error.message);
        if (on_error) return on_error(error, 422); else return;
    }
    ...
    if (result === undefined || result.type === undefined) {
        ...
        if (on_error) return on_error(new_error, 500); else return;
    }

    switch (result.type) {
        case "event":
            try {
                console.log("4.0 storing event from result: ", JSON.stringify(result, null, 2));
                let event = { data: result.data, name: result.name, summary: result.summary };
                console.log("4.1 persisting event");
                persist_event(event);
                if (on_event_success) on_event_success(result);
            } catch (error) {
                console.error("4.4 Error persisting event: " + error.message);
                if (on_error) return on_error(error, 500); else return;
            }
            break;
        case "exception":
            console.log("5.0 exception: ", JSON.stringify(result, null, 2));
            if (on_exception) on_exception(result);
            break;
        case "query":
            console.log("6.0 rendering query: ", JSON.stringify(result.query, null, 2));
            if (on_query) on_query(result);
            break;
        default:
            console.log("7.0 unknown result type: ", JSON.stringify(result, null, 2));
            if (on_unknown) on_unknown(result);
            break;
    }
}
...
function bootstrap(slices) {
    ...
    slices.forEach(slice => {
        console.log("bootstrapping slice: ", JSON.stringify(slice, null, 2));
        ...
        app_method(slice.navigation.path, (req, res, error_next) => {
            ...
            execute_slice_logic({
                slice: slice,
                parameter_function: () => { if (slice.navigation.web_data === undefined) return undefined; return slice.navigation.web_data(req); },
                on_error: (error, status) => { const new_error = new Error(error.message); new_error.status = status; return error_next(new_error); },
                on_event_success: (result) => {
            ...
            calculate_state(get_events, processor.todo_list.initial_state, processor.todo_list.event_handlers)
                .forEach(item => {
                    ...
                    if (processor.processor_filter(item)) {
                        ...
                        execute_slice_logic({
                            slice: slice,
                            parameter_function: () => { if (processor.processor_action === undefined) return undefined; return processor.processor_action(get_events, item); },
                            on_error: (error, status) => { return; },
                            on_event_success: (result) => { console.log("4.2 event stored successfully"); },
                            on_exception: (result) => { console.error("exception: ", JSON.stringify(result, null, 2)); },
                            on_query: (result) => { console.error("query result type not expected here: ", JSON.stringify(result, null, 2)); },
                            on_unknown: (result) => { console.error("unknown result type in result: ", JSON.stringify(result, null, 2)); }
                        });
                    }
                });
        }
        if (processor.execution === "immediate") {
            ...
            const timer = setInterval(() => {
                ...
                processor.todo_list.forEach(item => {
                    if (processor.processor_filter(item)) {
                        const result = processor.processor_action(item);
...
slices.push({
    name: "set_conference_name_default",
    navigation: { direction: "output", path: "/set-conference-name", view: "set-conference-name" }
});

slices.push({
    name: "name_the_conference",
    navigation: {
        direction: "input", path: "/set-conference-name", next_path: "/set-conference-name-confirmation",
...
slices.push({
    name: "conference_name_confirmation",
...
slices.push({
    name: "set_dates",
    navigation: {
        direction: "input", path: "/set-dates", next_path: "/set-dates-confirmation",
...
slices.push({
    name: "conference_dates_confirmation",
...
slices.push({
    name: "add_room",
    navigation: {
        direction: "input", path: "/rooms", next_path: "/rooms",
...
slices.push({
    name: "time_slots_addition",
    navigation: {
        direction: "input", path: "/time-slots", next_path: "/time-slots",
...
slices.push({
    name: "generate_conference_id_request_creation",
    navigation: { direction: "output", path: "/generate-conf-id", view: "generate-conf-id" },
});

slices.push({
    name: "generate_conference_id_request",
    navigation: {
        direction: "input", path: "/generate-conf-id", next_path: "/join-conference",
...
slices.push({
    name: "conference_id_generation_todo",
...
slices.push({
    name: "conference_id_generation_processor_action",
    ...
    initial_state: [false],
    processor: {
        execution: "immediate", todo_list_slice: "conference_id_generation_todo", triggering_events: ["conference_id_requested"],
...
slices.push({
    ...
    navigation: {
        direction: "output", path: "/register-success/:registration_id", view: "register-success", web_data: (req) => { return { registration_id: req.params.registration_id }; },
...
slice_tests.push({
    test_function: register_state_change,
    timelines: [
        {
...
slices.push({
    name: "topics",
    navigation: {
        direction: "output", path: "/topics/:registration_id", view: "topics", web_data: (req) => { return { registration_id: req.params.registration_id }; },
        access_checks: [participant_registered]
    },
...
slices.push({
    ...
    navigation: {
        direction: "output", path: "/topic-suggestion/:registration_id", view: "submit-session",
        access_checks: [participant_registered],
