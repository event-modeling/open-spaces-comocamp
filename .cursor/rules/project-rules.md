...
#### Structure Rules
- checkpoint arrays can either be used for input slices only or for output slices - they can't be mixed. The direction property in the navigation property identifies the slice. The check nodes only check the functionality of the slice the checkpoint array is in.
...
##### Complete Timeline Example for an output slice
```javascript
{
